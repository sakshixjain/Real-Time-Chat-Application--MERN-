...
function Form({isloggedin = false,})
 {
  ...
  useEffect(() => {
    ...
    const handleSubmit= async (e)=>{
      e.preventDefault();
       ...
       const url = `http://localhost:3000/api/${isloggedin ? 'login':'register'}`;
       const payload ={
        email:data.email,
  ...
  return (
    ...
    <div className='bg-[#08031B] shadow-[0_35px_60px_-15px_rgba(0.9,0.9,0.9,0.9)] border-x-4  w-[550px] h-[600px] rounded-3xl flex justify-center items-center flex-col '>
     ...
     {!isloggedin &&  <Input name='fullname' value={data.fullname} onchange={(e)=>{setdata({...data,fullname:e.target.value})}} type='text' placeholder='Enter the full name' label='Enter the full name' className='mb-3' />}
      ...
      <Input name='password' value={data.password} onchange={(e)=>{setdata({...data,password:e.target.value})}} type={hidePass ? "password":"text"} placeholder='Enter the password' label='Enter the password' className='mb-3 '/>
      <div>
                {hidePass ?<i onClick={handlepass} className="fa-regular fa-eye  text-stone-900 z-20  justify-center items-center fixed ml-24 -mt-10"></i> : <i onClick={handlepass} className="fa-regular fa-eye-slash  text-stone-900 z-20 -mt-10 justify-center items-center fixed ml-24"></i> }
